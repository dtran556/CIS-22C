...
public class Graph {
	 ...
	 * list at index u) @precondition, 0 > u & v, u & v <= vertices
	 * 
	 ...
	 * v @precondition, 0 > u & v, u & v <= vertices
	 * 
	...
	private void visit(int vertex) {
		...
		// System.out.println(color.get(adj.get(vertex).getIterator()));
		while (!adj.get(vertex).offEnd()) {
			if (color.get(adj.get(vertex).getIterator()) == 'W') {
